...
$(".ads_format2").prepend(`
   ...
   <div class="p-4 mt-2" style="padding-bottom:0px !important;">
      <img src="https://via.placeholder.com/200" alt="image" style="width:200px;height:150px;margin-bottom:1em;" id="image">
      <p id="title"></p>
   </div>
   <div class="row mb-4">
      <a class="btn col-md-6 downloadBtn adsButton">DOWNLOAD</a>
    </div>    
    </a>
</div>
...
$(".ads_format2 .closeBtn").click(function(){

	$(".ads_format2 .bc-banner").hide();
	$(".ads_format2 .adsDisplay").show();

});

$(".ads_format2 .stopBtn").click(function(){
	$(".ads_format2 .adsDisplay").hide();
  $(".ads_format2 .adsClosed").show();
  setTimeout(function(){$(".ads_format2 .adsClosed").fadeOut("slow")},500);
});

$(".ads_format2 .adsButton").click(function(){
  goToAd2();
});

async function getIpAd2(){
      ...
      async function submit_tracking_data_ad2(int_type){
        return await $.ajax({
      
          url:`${baseUrl}/submit_tracking_data.php?ad_type=ad_format_2&interaction_type=${int_type}&website=${website}&ip_address=${ip}&ad_id=${current_ad_id_ad2}`
    ...
    function myFunctionAd2() {
    
     var get_ads_request=getAdsData("ad_format_2");
     get_ads_request.then(res=>{
       var ad_data=JSON.parse(res);
       if(ad_data){
        $(".ads_format2").show();
        urlAd2=ad_data.link;
        current_ad_id_ad2=ad_data.id;
        $(".ads_format2 #title").html(ad_data.title);
        $(".ads_format2 #image").attr("src",ad_data.image);
        submit_tracking_data_ad2("view");
       }else{
         $(".ads_format2").hide();
       }
     });
    ...
    function goToAd2(){
        window.open(urlAd2,"_blank");
        submit_tracking_data_ad2("click").then(data=>{
            myFunctionAd2();
